...
class PieChartApp:
    ...
    def update_pie_chart(self, event):
        from_date = self.entry_from_date.get_date().replace(day=1).strftime('%Y-%m-%d')  # First day of selected month
        to_date = self.entry_to_date.get_date().strftime('%Y-%m-%d')  # Selected end date
