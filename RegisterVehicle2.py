...
class VehicleRegistrationGUI(cttk.CTkToplevel):
    ...
    def create_widgets(self):
        # Labels
        lbl_vehicle_name = cttk.CTkLabel(self.root, text="Vehicle Name:")
        lbl_vehicle_no = cttk.CTkLabel(self.root, text="Vehicle Number:")
        lbl_vehicle_owner = cttk.CTkLabel(self.root, text="Vehicle Owner:")
        lbl_owner_contact = cttk.CTkLabel(self.root, text="Owner Contact:")
        lbl_rfid_tag = cttk.CTkLabel(self.root, text="RFID Tag:")

        # Entry widgets
        self.entry_vehicle_name = cttk.CTkEntry(self.root)
        self.entry_vehicle_no = cttk.CTkEntry(self.root)
        self.entry_vehicle_owner = cttk.CTkEntry(self.root)
        self.entry_owner_contact = cttk.CTkEntry(self.root)
        self.entry_rfid_tag = cttk.CTkEntry(self.root, state='readonly')  # readonly for displaying RFID tag ID

        # Scanned RFID Tag ID Label
        lbl_scanned_rfid_tag = cttk.CTkLabel(self.root, text="Scanned RFID Tag:")
        self.lbl_scanned_rfid_tag_id = cttk.CTkLabel(self.root, text="", font=("Arial", 12, "bold"))

        # Simulated RFID Scanning Button
        btn_scan_rfid = cttk.CTkButton(self.root, text="Scan RFID", command=self.scan_rfid)

        # Register Button
        btn_register = cttk.CTkButton(self.root, text="Register", command=self.register_vehicle)
